<x-guest-layout>
    ...
    <h2 class="text-2xl font-bold text-center text-indigo-700 mb-6">Daftar ke Eventify</h2>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <input id="nama" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" name="nama" value="{{ old('nama') }}" required autofocus autocomplete="nama" />
            @error('nama')
    ...
    <!-- Tambah divider dan tombol Google register -->
    <div class="relative my-6">
        <div class="absolute inset-0 flex items-center">
            <div class="w-full border-t border-gray-300"></div>
        </div>
        <div class="relative flex justify-center text-sm">
            <span class="px-2 bg-white text-gray-600">Atau daftar dengan</span>
        </div>
    </div>

    <!-- Updated Google register button to use intent parameter -->
    <a href="{{ route('google.redirect', ['intent' => 'register']) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
