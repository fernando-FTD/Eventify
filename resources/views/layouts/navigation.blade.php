<nav x-data="{ open: false }" class="bg-indigo-700 border-b border-indigo-800 shadow-sm">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ url('/') }}" class="text-2xl font-bold text-white hover:text-indigo-100 transition">
                        Eventify
                    </a>
                </div>
                ...
                <div class="hidden space-x-6 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @guest
                        <x-nav-link :href="route('tentang')" :active="request()->routeIs('tentang')" class="text-white hover:text-indigo-100 border-indigo-500">
                            {{ __('Tentang Kami') }}
                        </x-nav-link>

                        <x-nav-link :href="route('bantuan')" :active="request()->routeIs('bantuan')" class="text-white hover:text-indigo-100 border-indigo-500">
                            {{ __('Bantuan') }}
                        </x-nav-link>
                    @endguest
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                @else
                    <div class="space-x-4">
                        <a href="{{ route('login') }}" class="text-sm text-white hover:text-indigo-200 font-medium">Masuk</a>
                        <a href="{{ route('register') }}" class="px-4 py-2 bg-white text-indigo-700 rounded-md text-sm font-bold hover:bg-indigo-50 transition shadow-sm">Daftar</a>
                    </div>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @guest
                <x-responsive-nav-link :href="route('tentang')" :active="request()->routeIs('tentang')" class="text-white hover:text-indigo-100 border-indigo-500">
                    {{ __('Tentang Kami') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link :href="route('bantuan')" :active="request()->routeIs('bantuan')" class="text-white hover:text-indigo-100 border-indigo-500">
                    {{ __('Bantuan') }}
                </x-responsive-nav-link>
            @endguest
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            @auth
                <div class="px-4">
                    <div class="font-medium text-base text-white">{{ Auth::user()->nama ?? Auth::user()->name }}</div>
            ...
            @else
                <div class="mt-3 space-y-1 px-4">
                     <x-responsive-nav-link :href="route('login')" class="text-white bg-indigo-800 rounded-md">
                        {{ __('Masuk') }}
                    </x-responsive-nav-link>
